...
| `cust_sub_category` | Many2one → `sales.cust_sub_category` | Domain: filter by cust_category |
| `cust_type` | Many2one → `sales.cust_type` | |
| `cust_area` | Many2one → `sales.cust_area` | |
| `tags` | Char | Google Maps address tag |
| `pkp` | Boolean | Pendapatan Kena Pajak (default=False) |
...
| `sold_to_ids` | One2many → `sales.sold_to` | Sold To addresses |
| `ship_to_ids` | One2many → `sales.ship_to` | Ship To addresses |
| `bill_to_ids` | One2many → `sales.bill_to` | Bill To addresses |
...
**Model: `sales.ship_to`** (`sales/models/models.py`)
...
**Model: `sales.sold_to`** (`sales/models/models.py`)

| Field | Tipe | Keterangan |
|-------|------|------------|
| `sold_id` | Char | Auto-generated |
| `sold_name` | Char | |
| `customer_id` | Many2one → `sales.customer` | ondelete=cascade |
| `address` | Text | |
| `country` → `district` | Many2one chain | Address hierarchy |

**Model: `sales.bill_to`** (`sales/models/models.py`)

| Field | Tipe | Keterangan |
|-------|------|------------|
| `bill_id` | Char | Auto-generated |
| `bill_name` | Char | |
| `customer_id` | Many2one → `sales.customer` | ondelete=cascade |
| `address` | Text | |
| `country` → `district` | Many2one chain | Address hierarchy |
...
| `sales.cust_sub_category` | `sub_category_id` (auto-gen), `sub_category_name`, `category_ref` → `cust_category` (hierarchy). **Blocked deletion** jika ada customer reference |
...
| `sales.cust_sub_category` | `sales_cust_sub_category` | `sub_category_id`, `sub_category_name`, `category_ref` |
| `sales.cust_type` | `sales_cust_type` | `type_id`, `type_name` |
| `sales.cust_area` | `sales_cust_area` | `area_id`, `area_name` |
| `sales.customer` | `sales_customer` | `customer_id`, `customer_name`, address hierarchy, `cust_category`, `cust_sub_category`, `cust_type`, `cust_area`, `tags`, `pkp`, `price_condition_ids`, `payment_terms`, `sold_to_ids`, `ship_to_ids`, `bill_to_ids`, `partner_id` |
| `sales.ship_to` | `sales_ship_to` | `ship_id`, `ship_name`, `customer_id`, address hierarchy |
| `sales.sold_to` | `sales_sold_to` | `sold_id`, `sold_name`, `customer_id`, address hierarchy |
| `sales.bill_to` | `sales_bill_to` | `bill_id`, `bill_name`, `customer_id`, address hierarchy |
...
| `sales.cust_sub_category` | Ada `sales.customer` yang reference | "Cannot delete sub category: it is used by customers" |
...
### 5.10 PKP Tax Validation on Sales Order

**Lokasi:** `sales/models/models.py` — `sales_order._check_pkp_taxes()`

**Trigger:** `@api.constrains('customer_id', 'order_line_ids')`

| Rule | Keterangan |
|------|------------|
| Jika `customer_id.pkp = True` | Semua SO line wajib memiliki `taxes` (tidak boleh kosong) |
| Jika `customer_id.pkp = False` | Tidak ada validasi pajak |

**Error Message:** "Customer '%s' is PKP (Pendapatan Kena Pajak). All order lines must have taxes assigned. Line with product '%s' does not have taxes."

### 5.11 Bill To Address Auto-fill on Invoice

**Lokasi:** `sales/models/models.py` — `sales_order._prepare_custom_invoice_vals()`

| Rule | Keterangan |
|------|------------|
| Jika `customer_id.bill_to_ids` terisi | Invoice `customer_address` diambil dari Bill To pertama |
| Jika `customer_id.bill_to_ids` kosong | Invoice `customer_address` diambil dari alamat customer (fallback) |
